...
describe("members", () => {
  ...
  test("바인딩된 멤버를 언바인드하면 user_email 이 지워진다", () => {
    const claimed = m.claimMember({ studentNo: "2099310001", email: "a@b.com" });
    expect(claimed.ok).toBe(true);
    if (!claimed.ok) return;

    const r = m.unbindMember(claimed.member.id, "admin@b.com");
    expect(r.ok).toBe(true);
    expect(m.getMemberByStudentNo("2099310001")?.user_email).toBeNull();
  });
  ...
  test("언바인드된 학번은 다른 계정이 다시 클레임할 수 있다", () => {
    ...
    const r = m.claimMember({ studentNo: "2099310001", email: "z@z.com" });
    expect(r.ok).toBe(true);
    expect(m.getMemberByEmail("z@z.com")?.student_no).toBe("2099310001");
  });

  test("바인딩되지 않은 멤버를 언바인드하면 실패", () => {
    const unbound = m.getMemberByStudentNo("2099310002");
