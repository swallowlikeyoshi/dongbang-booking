...
describe("isValidStudentNo", () => {
  it("accepts a well-formed 10-digit ID", () => {
    expect(isValidStudentNo("2099310001")).toBe(true);
  });
  ...
  it("rejects 11 digits", () => {
    expect(isValidStudentNo("20993100011")).toBe(false);
  });
  ...
  it("rejects a non-string value", () => {
    expect(isValidStudentNo(2099310001)).toBe(false);
...
describe("validateClaimInput", () => {
  it("passes with a well-formed studentNo only", () => {
    const r = validateClaimInput({ studentNo: "2099310001" });
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.studentNo).toBe("2099310001");
  });

  it("fails a subTeam outside SUB_TEAMS", () => {
    const r = validateClaimInput({ studentNo: "2099310001", name: "홍길동", subTeam: "없는팀" });
    expect(r.ok).toBe(false);
  });

  it("passes for each of the four valid SUB_TEAMS values", () => {
    for (const t of SUB_TEAMS) {
      const r = validateClaimInput({ studentNo: "2099310001", name: "홍길동", subTeam: t });
      expect(r.ok).toBe(true);
  ...
  it("fails an empty name", () => {
    const r = validateClaimInput({ studentNo: "2099310001", name: "" });
    expect(r.ok).toBe(false);
  });
  ...
  it("fails a 21-character name", () => {
    const r = validateClaimInput({ studentNo: "2099310001", name: "a".repeat(21) });
    expect(r.ok).toBe(false);
  });

  it("passes a 20-character name", () => {
    const r = validateClaimInput({ studentNo: "2099310001", name: "a".repeat(20) });
    expect(r.ok).toBe(true);
  });
});
